...
public class OnTap_CuoiKy extends AppCompatActivity {
    ...
    private void loadAdapter(){ // Load giao diên và load ảnh trog lítview
        adapter = new MyAdapter(this,R.layout.custom_row, listImg);
        listView.setAdapter(adapter);
    }

    @Override
    public boolean onCreateOptionsMenu(Menu thisMenu) { //Khởi tạo cho icon xuất hiện trên giao diện chính
        getMenuInflater().inflate(R.menu.menu, thisMenu);
        return super.onCreateOptionsMenu(thisMenu);
    }

    @Override
    public boolean onOptionsItemSelected(MenuItem item) { // Khi nhấn chọn vào case nào thì thực hiện chức năng mới mục đchs tương ứng trên menu
        int id = item.getItemId(); // Lấy ifd địa chỉ hình nhấn vào khi nhấn adđ -> theem
        switch (id){
            case R.id.menu_Add:
    ...
    private void Add(){
        ...
        dialog.setContentView(R.layout.dialog_add); // Lấy giao diện dialog
            ...
            public void onClick(View view) {
                ...
                listAllImages.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> adapterView, View view, int i, long id) {
                        ...
                        pathImg = arr_image.get(i).getImgID(); // Lấy hình ảnh từ trog mảng arr-img tại vị trí đaã chọn
                        view.setBackgroundResource(R.color.colorAccent);
                        btTake.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View view) {
                                pic.setImageResource(pathImg);// Imgview ở dialog thứ 1 sẽ gán giá trị hình ảnh của pathImg
                                imgDialog.dismiss(); // Take - đóng
                                }
                            });
                    }
                });
        ...
        btAdd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(pathImg == 0){
                    ...
                    loadAdapter(); // Load lại để trên layout main chính nhân
                    pathImg = 0; //reset lại id của hình
                    dialog.dismiss();//đóng dialog thứ 1
